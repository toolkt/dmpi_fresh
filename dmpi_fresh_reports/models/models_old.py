...
class DmpiCrmTemplate(models.Model):
	...
	_sql_constraints = [
		('unique_template', 'UNIQUE (name)', _('Similar tempalte already exists!'))
	]
...
class DmpiCrmTemplateLine(models.Model):
	...
	factor_id = fields.Many2one('dmpi.crm.factor', string='Factor', required=True)
...
class DmpiCrmFactor(models.Model):
	...
	type = fields.Selection([('external','External Quality'),('internal','Internal Quality'),('packaging','Packing Quality')], string='Operation', required=True)
	inactive = fields.Boolean('Inactive', default=False)
...
class DmpiCrmPreshipReport(models.Model):
	...
	dr_id = fields.Many2one('dmpi.crm.dr', 'DR Reference', required=True)
	tmpl_id = fields.Many2one('dmpi.crm.template', 'Pre-Shipment Template', required=True)
