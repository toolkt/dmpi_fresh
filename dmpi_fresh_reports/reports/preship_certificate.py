...
class PreShipmentCertificateReport(models.AbstractModel):
	...
	def generate_xlsx_report(self, workbook, data, obj):
		for o in obj:
			sheet = workbook.add_worksheet('Pre-Ship Cert %s' % o.dr_id.name)
			...
			head_dash = workbook.add_format({'font_size': 8, 'font_name':'Arial', 'align':'center', 'border':3})
			...
			for rec in result:
				...
				rule = rec['rule']
				clss = self._get_class(rule, weight, score) if rule else ''
				...
				if rec['type'] == 'external':
					...
					if col == (col_end + 1):
						if grp_clss != 'HOLD':
							...
							grp_clss = self._get_class(grp_rule, grp_weight, grp_score)
			...
			# WRITE TOTALS
			total_weight = o.tmpl_id.total_weight
			overall_rule = o.tmpl_id.overall_rule
			print (total_clss)

			if total_clss != 'HOLD':
				total_clss = self._get_class(overall_rule, total_weight, total_score)
			print (total_clss)
