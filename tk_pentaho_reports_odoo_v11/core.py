...
def get_proxy_args(instance, cr, uid, prpt_content, context_vars={}):
    ...
    xml_interface = IRConfig.get_param('pentaho.odoo.xml.interface', default='').strip()
    xml_port = IRConfig.get_param('pentaho.odoo.xml.port', default='').strip()
...
#         self.lang_dict_called = False
    ...
    def setup_report(self):
        ...
        report = env['ir.actions.report'].search([('report_name', '=', self.name[len(SERVICE_NAME_PREFIX):]), ('report_type', '=', 'pentaho')], limit=1)
        if not report:
...
class PentahoReportOdooInterface(report_int):
    ...
    def create(self, cr, uid, ids, data, context):
        ...
        report_xml = env['ir.actions.report'].search([('report_name', '=', name[len(SERVICE_NAME_PREFIX):])], limit=1)
...
class PentahoReportDownload(http.Controller):
    ...
    def download_report(self, data, token):
        ...
        pdfhttpheaders = [('Content-Length', len(res)),('Content-Type', 'application/download')]
