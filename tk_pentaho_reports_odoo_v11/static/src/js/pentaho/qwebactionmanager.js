...
ActionManager.include({
    ir_actions_report: function (action, options) {
        ...
        if (action.report_type === 'qweb-html') {
            ...
            (wkhtmltopdf_state = wkhtmltopdf_state || this._rpc({route: '/report/check_wkhtmltopdf'})).then(function (state) {
                ...
                if (state === 'upgrade' || state === 'ok') {
                    ...
                    do {
                        ...
                        console.log(self.getSession(), response, c, current_action, options);
                        if (!success) {
        ...
        } else if (action.report_type === 'pentaho') { 
            Model.query({ 
                ...
                return self.getSession().get_file({
            ...
            console.log(action);
            // trigger_download(self.getSession(), response, c, action, options);
            return;
        }
    }
});
